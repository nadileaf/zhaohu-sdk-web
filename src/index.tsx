...
function DemoComponent() {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div>
        <label className="label-content">
          <div className="label">品宣职位id:</div>
          <input name="adJobId" value={inputs.adJobId} onChange={handleInput}></input>
        </label>
      </div>
      <div>
        <label className="label-content">
          <div className="label">直聊职位id:</div>
          <input name="chatId" value={inputs.chatId} onChange={handleInput}></input>
        </label>
      </div>
      <div>
        <label className="label-content">
          <div className="label">Token:</div>
          <textarea name="token" value={inputs.token} onChange={handleInput} rows={4} cols={50}></textarea>
        </label>
      </div>
      <div>
        <label className="label-content">
          <div className="label">初始化简历:</div>
          <textarea name="resume" value={inputs.resume} onChange={handleInput} rows={20} cols={50}></textarea>
        </label>
      </div>
      ...
      <div className="label-content">
        ...
        <label><input type="radio" name="mode" value="" checked={inputs.mode === ""} onChange={handleInput}/>common</label>
        <label><input type="radio" name="mode" value="hide" checked={inputs.mode === "hide"} onChange={handleInput}/>hide</label>
      </div>

      <div className="button-group">
        <button className="button" type="submit">召乎一下</button>
        {
          zhaohu ?
          (<button className="button" type="button" onClick={handleSwitch}>{open ? 'hide' : 'show'}</button>) :
          (undefined)
        }
      </div>
    </form>
